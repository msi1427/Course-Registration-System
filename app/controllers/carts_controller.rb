class CartsController < ApplicationController
  ...
  def destroy
    ...
    #session[:cart_id] = nil
    respond_to do |format|
      format.html { redirect_to carts_path, notice: 'Registration Cancelled' }
  ...
  def user_authenticate
    @cart.line_items.each do |it|
        if current_user.id == it.user.id
          return true
          break
        end
      end
  ...
  def user_admin_authenticate
    if current_user.role == 'Admin' or user_authenticate
      return true
    else
    return false
    end
